...
def rand(n: int) -> Vector:
    """Create a new Vector of length `n` with uniform random values in [0, 1)."""
    ...

def randn(n: int) -> Vector:
    """Create a new Vector of length `n` with standard normal (Gaussian) random values."""
    ...

def linspace(start: float, stop: float, num: int) -> Vector:
    """Create a new Vector with `num` evenly spaced values from `start` up to `stop`."""
    ...
